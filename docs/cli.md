...
The CLI entry point is `gen`, with four subcommands: `ledger`, `auto`, `rippled`, and `compose`.
...
# Parse from a local rippled features.macro
...
Generate validator configurations (rippled.cfg files with UNL and voting stanzas).
