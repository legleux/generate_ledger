...
**Created**: 2025-12-10
**Status**: In Progress - MVP + AMM Complete (US1-3, US5), Remaining: MPT (US4), Vault (US6)
...
- **AMM Implementation Complete**: Full AMM pool generation (amm.py, 472 lines), 32 tests, CLI support (`--amm-pool/-a`). Genesis-based approach — no transaction initialization needed.
- **Amendment System Overhauled**: Profile-based loading (release/develop/custom), features.macro parser, `amendment_hash()`, per-amendment overrides. 92 amendments in release profile.
- **Test Suite Expansion**: 46 → 253 tests across 18 test files. Includes rippled-verified test vectors, AMM regression tests, full pipeline integration tests.
- **Stable vs Develop Separation**: `develop/` package with object registry, MPT/Vault stubs. Branch strategy: main=stable, develop=pre-release.
