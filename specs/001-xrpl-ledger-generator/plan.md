...
1. **XRPL Genesis Ledger Format**: ✅ RESOLVED - See research.md. Structure implemented in src/generate_ledger/ledger_builder.py with AccountRoot, RippleState, AMM, FeeSettings, and Amendments objects.
2. **MPT Creation**: 🔴 DEFERRED to v2.0 - Requires additional research on MPTokenIssuance and MPToken ledger object specifications.
3. **AMM Initialization**: ✅ RESOLVED - Genesis-based implementation completed in src/generate_ledger/amm.py (472 lines). AMM pools pre-generated with proper indices, LP token accounting, and pseudo-account derivation. No transaction-based initialization needed.
