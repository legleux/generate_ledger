...
# from generate_ledger.ledger import LedgerNamespace
...
def compute_account_index(account_id):
    return compute_index(account_id, b"\x00\x61")


def order_low_high(a1: bytes, a2: bytes) -> tuple[bytes, bytes]:
    """
...
def currency_to_160(code: str) -> bytes:
    ...
    (XRP is all-zero currency, but XRP cannot be a trust-line currency.)
    """
    code = code.strip()
...
def compute_index(input_data, space_key):
