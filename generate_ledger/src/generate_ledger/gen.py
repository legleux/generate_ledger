...
def compute_index(data, space_key: LedgerNamespace) -> LedgerIndex:
    # TODO@legleux: Clean up chained methods
    data_hex = base58.b58decode_check(data, alphabet=base58.XRP_ALPHABET)[1:].hex().upper()
    message = hashlib.sha512(bytes.fromhex(space_key.hex + data_hex))
    object_index = message.digest()[:32].hex().upper()
    return object_index


wallet = xrpl.wallet.Wallet.from_seed(xrpl.core.keypairs.generate_seed())
...
def write_ledger_file():
    ...
    ledger_file = Path("test_network/ledger.json")
