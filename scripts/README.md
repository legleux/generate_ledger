...
usage: bench_accounts.py [-h] [-n COUNT] [--mode {seq,mp,thread,hybrid,gpu}]
                         ...
                         [--seeds-file FILE] [--save-seeds FILE] [--seeds-only]
...
Account Options:
  ...
  --seeds-file      Load pre-generated seeds from file (auto-detects format)
  --save-seeds      Save generated seeds to file
  --seeds-only      Only generate seeds, skip derivation (requires --save-seeds)
  --seeds-text      Save seeds as text instead of binary (default: binary)
...
Trustline Options:
  ...
  --trustlines-only Skip account benchmark, use --seeds-file for addresses
```
...
| Format | Description | Size (100k seeds) |
|--------|-------------|-------------------|
| Binary (default) | Magic header + raw bytes | ~1.6 MB |
| Text | One hex string per line | ~3.3 MB |

Binary format is auto-detected when loading. Use `--seeds-text` to save as text for
compatibility with other tools or manual inspection.

```bash
# Save as binary (default, faster I/O)
uv run scripts/bench_accounts.py -n 100000 --seeds-only --save-seeds seeds.bin

# Save as text (for compatibility/inspection)
uv run scripts/bench_accounts.py -n 100000 --seeds-only --save-seeds seeds.txt --seeds-text

# Loading auto-detects format
uv run scripts/bench_accounts.py --seeds-file seeds.bin --mode mp
...
# Pre-generate seeds for reproducible benchmarks
uv run scripts/bench_accounts.py -n 1000 --save-seeds seeds.txt
uv run scripts/bench_accounts.py --seeds-file seeds.txt --mode mp
uv run scripts/bench_accounts.py --seeds-file seeds.txt --mode thread

# Generate seeds only (no derivation) - useful for large seed files
uv run scripts/bench_accounts.py -n 100000 --seeds-only --save-seeds seeds.txt

# Load seeds from previous benchmark output
uv run scripts/bench_accounts.py --seeds-file results.json --mode seq
...
# Benchmark trustlines only (reuse accounts from file)
uv run scripts/bench_accounts.py -n 1000 --save-seeds accounts.json --output accounts.json
uv run scripts/bench_accounts.py --seeds-file accounts.json --trustlines-only --mode mp
```
